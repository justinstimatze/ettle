...
## What it tests (the stale-assumption tangle kind, in isolation)

`KindStaleAssumption` (mesh.go) is the only named tangle kind with no dedicated
...
- **K1 (real).** Mira's rollup reads `events.user_id` as a stable bigint and
  ...
  tangle is the invalidation. This is the recall target.

The catch: a keyword matcher should *also* light up on the decoy, which shares
more surface tokens than the real tangle does.
...
- **D1 (decoy).** Priya works on the *same feature* as Mira (the DAU dashboard)
  ...
  depends on, so nothing of hers can go stale. Surfacing a tangle here is the
...
ask: a firm tangle matching D1 is reported as a trap hit — ettle inventing a
