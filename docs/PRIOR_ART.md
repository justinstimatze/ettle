...
Five passes: a knowledge-based review of the academic lineage, a shallow live web pass (June 2026), a second shallow pass (June 2026, §7) prompted by the friction/empowerment alignment and the deliberation-routing the team sim now does, and a third (June 2026, §9) prompted by shipping the MCP surface — which found the agent-fed-standup *wrapper* now shipped commercially while cross-person knot-reconciliation did not, and a fifth (July 2026, §10) that is not a search at all but a close read of one primary source — a Dust co-founder stating ettle's diagnosis and answering it with pooled shared state. The deep-research harness was considered and rejected on cost each time.
...
- **Atlassian Rovo, Microsoft 365 Copilot, Dust, Notion AI, Slack/Agentforce** — agents over org data. Adjacent. **Dust** is the exception that earned its own section: it states ettle's single-player→multiplayer diagnosis explicitly and answers it with pooled shared state. See §10.
...
## 10. Dust's "pods" (July 2026) — the multiplayer thesis, answered with shared state

Not a search pass: a close read of one primary source, an [MLOps Community Podcast](https://home.mlops.community/) episode with a **Dust** co-founder (ex-Stripe, ex-OpenAI). Worth its own section because it is the clearest public statement of ettle's *problem* by someone shipping a *different answer to it*. Quotes below are from an auto-generated transcript and carry ASR noise (it renders "Stripe" as "scribe" and "work" as "walk" throughout); they are quoted as-transcribed and used for framing, not as precise wording.

**(a) The diagnosis is ettle's, stated plainly.** The thesis is single-player → multiplayer AI: today "most of the agent interactions we have, we just have them alone," and — the line that matters most for ettle — *"you share the output, you don't share the session."* That is exactly ettle's L1 premise: sessions are jagged and messy, so what travels between people is the distilled artifact, not the trace. Together with §9(d)'s Ace and §3's DailyBot, this is now three independent, well-resourced teams naming the same bottleneck in 2026. The diagnosis is no longer the differentiator.

**(b) The mechanism is shared mutable state — the opposite architectural bet.** Dust's answer is the **pod**: a group of humans, agents, and sessions over a shared filesystem (per-session state plus a `/pod` directory, GCS-fuse-backed, agents free to move files between the two). The worked example is a team weekly an agent orchestrates end-to-end — one session per slide, per owner, collated hours before the meeting. It is a genuinely good product, and it is the richest version of §3's "put everything in one place" answer. But it makes coordination easy by **dissolving** it: everything is pooled, everyone can read and write everything (*"in Google Drive you can modify whatever else has been doing in your company as well, so I think it's okay"*), and there is no undo as of the interview. ettle's bet is the inverse — the interesting coordination cases are precisely the ones where state *cannot* or *should not* be pooled, which is why the contextual privacy boundary is an invariant and reconciliation is a real problem rather than a directory listing. Two systems, same diagnosis, opposite premise about the boundary.

**(c) The interview contains its own counter-argument, twice.**
- *Concurrency.* Asked about history/undo: *"who uses file locking? [laughter]"* — answered moments later with *"agents will."* Minutes on, the same speaker reaches for git-as-substrate for agent fleets, because *"you have that uh resolution forced resolution step when you merge back to main."* A shared filesystem with N agents rediscovers version control; ettle's [leat](https://github.com/justinstimatze/leat) transport (append-only per-author lanes, LWW per key, spoof-dropping) starts there rather than arriving there.
- *Agent-native infra.* The stated principle is that the technology is *"extremely extremely extremely anthropomorphic"* — trained on human traces — so give agents the tools humans use (his example: agentic search displacing RAG as context windows grew). Then the pod itself is agent-native infrastructure that no human asked for. The heuristic is right about **interfaces** and wrong about **memory**: what agents lack is not hands, it is continuity across sessions and across people.

**(d) The load-bearing disagreement: what causes multiplayer.** His causal story is capability-driven — we are single-player because agent task horizons top out around half a day, and multiplayer arrives when horizons reach a week, because week-long tasks are "inherently cross team cross people." ettle's account is that coordination failure is caused by **distributed private information and divergent incentives**, not task duration: two people diverge on a twenty-minute task if they hold different facts about the same thing. Longer horizons do not create the multiplayer problem; they make it expensive enough to notice. This is not a quibble — it is why ettle's *useful at N=1* invariant is coherent and why ettle does not have to wait for the METR horizon curve to move before it is worth anything.

**(e) Two things worth taking.** The **bidirectional-access principle** — nothing a human can do that an agent cannot, and vice versa — is a good operating constraint that ettle largely already satisfies through the MCP seam (`ettle room` / `standup` for humans, `ettle_horizon` / `ettle_respond` for agents, over the same knots); it is worth holding as an explicit invariant rather than an accident. And the **"fog of AI"** answer to planning under a moving substrate — refuse to plan past six months, but never refuse to paint a picture; ship a three-month direction with high conviction and an explicit expectation of rewriting it at month three — is a sharper articulation of what the README status section already does.

*Recency note: single-source, from a podcast; product facts (pods, GCS-fuse backing, no undo) are as described by the speaker in mid-2026 and will move. The durable finding is the positioning: Dust answers multiplayer AI with a shared workspace and pooled state; ettle answers it with bounded per-person state and automated cross-person reconciliation. Same problem, opposite premise.*
