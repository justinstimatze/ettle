...
**(c) The bind-vs-surface routing is no longer empty space — apply it, don't invent it.** This is the most useful finding for the design just built. "Process the uncontroversial efficiently; deliberate the contested" is now a published protocol shape and a shipped product:
- **Deliberative Curation** ([arXiv 2606.00007](https://arxiv.org/html/2606.00007)) — explicitly: most contributions are uncontroversial and should be processed efficiently, contested/high-stakes ones get progressively more deliberation. That *is* ettle's friction-in-the-right-spots, as a multi-agent protocol.
