...
**0c. Interrogative register for unconfirmed cross-person knots** *(situated-action lens; extraction-skepticism lens).* The FIRM/soft split stays *internally* (it is a good ranking signal). But the **output register** for any cross-person knot that has not yet been confirmed by its parties becomes a question addressed to the pair — "Ivo, Mara: you may have a collision about the pricing package; here is each of your stated positions — is this real?" — not "[collision] … a direct conflict." This is a template + prompt change, not a logic change; the detector still detects, it just stops *asserting* what it has no standing to assert. Cost: surface templates + the routing-to-pair the demo already half-does. Limit: see the tension on "toil → zero" below — this is where the headline gets paid for.
